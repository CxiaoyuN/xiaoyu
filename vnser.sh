...
if [ ! -d "${APP_PATH}openssl" ] ;then
	...
	wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/openssl-1.0.0e.tar.gz  >/dev/null 2>&1
...
wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/lzo-2.03.tar.gz  >/dev/null 2>&1
...
if [ ! -d "${APP_PATH}openvpn" ] ;then
	...
	cd ~
	wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/mproxy.c &> /dev/null
	gcc -o mproxy mproxy.c
	chmod 777 ./mproxy &> /dev/null
	
	echo -e '\n\033[36m正在编译安装openvpn,请稍等(大概需要20-30秒)...\033[0m'
	cd ${APP_PATH}
	wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/openvpn-2.3.12.tar.gz  >/dev/null 2>&1
	tar -xvzf openvpn-2.3.12.tar.gz  >/dev/null 2>&1
	cd openvpn-2.3.12
	./configure --prefix=/usr/local/openvpn  >/dev/null 2>&1
	 make install >/dev/null 2>&1
	#安装easy-rsa文件
	wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/easy-rsa.zip  >/dev/null 2>&1
...
echo -e "\n\033[31m Openvpn配置成功 \033[0m"	
	...
	wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/hook.zip &> /dev/null
	...
	#下载重启文件
	wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/vnser &> /dev/null
	...
	if [ "${is_lk}" != "n" ];then
		...
		wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/install.zip &> /dev/null
		...
		wget http://git.oschina.net/xiaoyutwo/vnser/raw/master/vnser_lk.zip &> /dev/null
...
APP:http://git.oschina.net/xiaoyutwo/vnser/raw/master/vnser.apk
MT管理器:http://git.oschina.net/xiaoyutwo/vnser/raw/master/MT.apk
对接教程：http://git.oschina.net/xiaoyutwo/vnser/raw/master/readme.txt
